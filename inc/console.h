...
/** \brief Function to clear terminal screen.
 *
 * Prints ASCII FF (form feed) character, which clears terminal screen
 * Tested with PuTTY terminal
 *
 *
...
/** \brief Function to flush receive buffer, to avoid unwanted commands that were transmitted when console was not ready
 *
 *
 * \pre only works if init() previously called
 */
